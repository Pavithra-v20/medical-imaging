...
def _resize_mask(mask: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    """Resize a binary mask to match (width, height)."""
    if mask is None:
        return None
...
def _preprocess(image: Image.Image, mask: np.ndarray | None):
    ...
    # Normalize and apply XRV transforms
...
async def classify_xray(image_path: str, mask: np.ndarray | None = None) -> dict:
    """
    Run a pretrained TorchXRayVision DenseNet on a 2D image.
    Returns a dict compatible with the prediction schema.
    """
